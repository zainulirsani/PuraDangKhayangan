...
export default function VirtualTourPage() {
  ...
  return (
    <div className="min-h-screen bg-secondary">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-4">
              Tur Virtual <span style={{ color: '#B59410' }}>Pura</span>
            </h1>
            ...
            <div className="lg:col-span-8">
              <Card className="overflow-hidden shadow-lg">
                <CardHeader className="flex flex-row items-center justify-between">
                  ...
                  <Badge variant="outline">Tampilan 360°</Badge>
                </CardHeader>
                ...
                <CardContent>
                  <div className="space-y-6">
                    {tourSpots.map((spot, index) => (
                      ...
                      >
                        <h4 className={`font-semibold transition-colors ${currentSpot === index ? "text-foreground" : "text-muted-foreground group-hover:text-foreground"}`}>
                            {spot.name}
                        </h4>
                        <p className={`text-sm mt-1 line-clamp-2 ${currentSpot === index ? "text-muted-foreground" : "text-gray-400 group-hover:text-muted-foreground"}`}>{spot.description}</p>
                         {currentSpot === index && <div className="mt-2 h-0.5 rounded-full" style={{ backgroundColor: '#B59410' }} />}
                      </div>
                    ))}
                  </div>
                </CardContent>
      ...
      <Navigation currentPage="/virtual-tour"/>
    </div>
  )
}